...
import { Heart, MapPin, Fuel, Gauge, Settings, Eye, Scale } from 'lucide-react';
    ...
    <Link to={`/car/${car.id}`} className="block group h-full">
      <div className="luxury-card h-full flex flex-col overflow-hidden hover:shadow-md transition-shadow duration-200">
        ...
        <div className="relative aspect-[16/10] overflow-hidden bg-muted">
          ...
          {car.views > 0 && (
            <div className="absolute bottom-2 right-2 flex items-center gap-1 text-white/90 text-[10px] backdrop-blur-sm bg-black/30 px-1.5 py-0.5 rounded-sm">
              <Eye className="w-3 h-3" /> {car.views}
            </div>
          )}
        </div>
